...
class _3DMM:
    ...
    def opt_3DMM_fast(self, Weights, Components, Components_res, id_landmarks_3D, landImage, avgFace,
                      _lambda, rounds, r, C_dist):
        num_landmarks = len(landImage)
        _Weights = Weights
        index = np.array(id_landmarks_3D, dtype=np.intp)
        app_var = np.reshape(avgFace[index, :], (num_landmarks, 3), order='F')
        ...
        _defLand = np.reshape(_defShape[index, :], (num_landmarks, 3), order='F')
        ...
        # euclidean distance between the projected landmarks and the 2D landmarks using numpy
        reproj_error = np.linalg.norm(landImage - _proj, axis=1)
        #print(f'Error: {np.mean(reproj_error)}')
