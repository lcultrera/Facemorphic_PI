...
class FacemorphicDataset(Dataset):
    def __init__(self, data_dir, split, mode='event', task='AU', max_seq_len=75, central_crop=True, transform=None, toy=False, 
                 clean_cache=False, use_cache=True, do_pad=True, au_labels=None, use_annot=False,disaligned='rgb', disalignment_max=0):
        ...
        self.disalignment_max=disalignment_max
        #self.disaligned = disaligned # 'rgb' or 'event' or None
        self.rgb_disaligned = disaligned == 'rgb'
        self.event_disaligned = disaligned == 'event'
    ...
    def __get_event(self, idx):
        ...
        if self.event_disaligned:
            #print(event_imgs.shape)
            event_imgs = np.pad(event_imgs, ((self.disalignment_max,0),(0,0),(0,0),(0,0)))
            #print(event_imgs.shape)
        if self.do_pad:
    ...
    def __get_rgb(self, idx):
            ...
            if self.rgb_disaligned:
                print(rgb_imgs.shape)
                rgb_imgs = np.pad(rgb_imgs, ((self.disalignment_max,0),(0,0),(0,0),(0,0)))
                print(rgb_imgs.shape)
            if self.do_pad:
