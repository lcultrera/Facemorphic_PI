    ...
    for epoch in range(num_epochs):
        ...
        for n, batch in tqdm(enumerate(dataloader_train)):
            ...
            with torch.cuda.amp.autocast():
                ...
                loss_PI = loss_fn_mse(rgb_feature, event_feature) #loss between event and RGB
                loss_au = loss_fn(outputs, labels)
...
class CombinedModel(nn.Module):
    def __init__(self, model_event, model_rgb, num_classes):
        super(CombinedModel, self).__init__()
        self.model_event = model_event
        self.model_rgb = model_rgb
        self.model_rgb2 = model_rgb
...
loss_fn = torch.nn.CrossEntropyLoss(), mode=params['mode'], task=params['au'], toy=True, max_seq_len=params['max_seq_len'], transform=data_transform, use_annot=True, use_cache=True)
dataset_test = FacemorphicDataset('/home/lcultrera/FACEMORPHIC', split='test', 
#define mse loss
